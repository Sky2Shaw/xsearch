...
def test_synthesize_modules(tmp_path):
    fixtures = Path(__file__).parent / "fixtures"
    graph = parse_stage1(fixtures)
    outputs = synthesize(graph, output_dir=tmp_path / "stage2_outputs")
    assert "ontology" in outputs
    assert "modules.yaml" in outputs["ontology"]


def test_module_inference(tmp_path):
    fixtures = Path(__file__).parent / "fixtures"
    graph = parse_stage1(fixtures)
...
def test_validator_generation(tmp_path):
    fixtures = Path(__file__).parent / "fixtures"
    graph = parse_stage1(fixtures)
    output_dir = tmp_path / "stage2_outputs"
    synthesize(graph, output_dir=output_dir)
    ...
    for v_name in ["ub_capacity", "l1_capacity", "workspace_no_alias"]:
        v_path = output_dir / "validators_spec" / f"{v_name}.yaml"
        assert v_path.exists(), f"Missing validator {v_name}"

    # Should have risk-derived validator
    risk_v_path = output_dir / "validators_spec" / "valid_r_test_1.yaml"
    assert risk_v_path.exists()


def test_synthesizer_writes_agent_ready_artifacts(tmp_path):
    fixtures = Path(__file__).parent / "fixtures"
    graph = parse_stage1(fixtures)
    output_dir = tmp_path / "stage2_outputs"
    synthesize(graph, output_dir=output_dir)
...
def test_synthesizer_adds_agent_metadata_to_schema_fields(tmp_path):
    fixtures = Path(__file__).parent / "fixtures"
    graph = parse_stage1(fixtures)
    output_dir = tmp_path / "stage2_outputs"
    synthesize(graph, output_dir=output_dir)
